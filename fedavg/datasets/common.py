...
from .leaf import CelebA, FEMNIST
...
dataset_cfg = {
    ...
    "FEMNIST": {
        'std': (0.15767843198426892,),
        'mean': (0.9642384386141873,),
        'inv_std': (1/0.15767843198426892),
        'inv_mean': (-0.9642384386141873/0.15767843198426892,)
    },
     "CelebA": {
        'std': (0.30725626, 0.28689492, 0.28622603),
        'mean': (0.50419323, 0.42377321, 0.38135761),
        'inv_std': (1/0.30725626, 1/0.28689492, 1/0.28622603),
        'inv_mean': (-0.50419323/0.30725626, -0.42377321/0.28689492, -0.38135761/0.28622603)
    },
    "FashionMNIST": {
        'std': (0.3530,),
        'mean': (0.2860,),
...
def get_dataset_by_name(dataset, train_batch_size, test_batch_size, k_batches=1, rng=None, normalize=True, maxn=None, n_clients=None, same_cls=False):
    if dataset.startswith('dist_'):
        ...
        dummy_labels = np.random.randint(0, n_targets, (train_batch_size))
        ...
        train_data = FEMNIST('../data', train=True, maxn=maxn, same_cls=same_cls, transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.9642384386141873,), (0.15767843198426892,)) if normalize else ToJNP(),
            ToJNP()]))
        test_data = FEMNIST('../data', train=False, maxn=maxn, same_cls=same_cls, transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.9642384386141873,), (0.15767843198426892,)) if normalize else ToJNP(),
            ToJNP()]))
        ...
        dummy_labels = np.random.randint(0, n_targets, (train_batch_size))
        if n_clients is None:
            train_loader_net = torch.utils.data.DataLoader(train_data, batch_size=1, collate_fn=numpy_collate, shuffle=True)
            test_loader_net = torch.utils.data.DataLoader(test_data, batch_size=1, collate_fn=numpy_collate, shuffle=False)
            train_loader_def = torch.utils.data.DataLoader(train_data, batch_size=1, collate_fn=numpy_collate, shuffle=True)
            test_loader_def = torch.utils.data.DataLoader(test_data, batch_size=1, collate_fn=numpy_collate, shuffle=False)
            return (train_loader_net, test_loader_net), (train_loader_def, test_loader_def), n_targets, (dummy_input, dummy_labels)
        else:
            (train_loaders, test_loaders), (train_loaders_def, test_loaders_def) = split_data(train_data, test_data, 1, 1, 1, n_clients)
            return (train_loaders, test_loaders), (train_loaders_def, test_loaders_def), n_targets, (dummy_input, dummy_labels)
    elif dataset == "CelebA":
        train_data = CelebA('../data', train=True, transform=transforms.Compose([
                                                       transforms.ToTensor(),
                                                       transforms.Resize([84,84]),
                                                       transforms.Normalize((0.50419323, 0.42377321, 0.38135761), (0.30725626, 0.28689492, 0.28622603)),
                                                       ToJNP()]))
        test_data = CelebA('../data', train=False, transform=transforms.Compose([
                                                        transforms.ToTensor(),
                                                        transforms.Resize([84,84]),
                                                        transforms.Normalize((0.50419323, 0.42377321, 0.38135761), (0.30725626, 0.28689492, 0.28622603)),
                                                        ToJNP()]))
        n_targets = 2
        dummy_input = np.einsum("bijk -> bjki", np.random.rand(*(train_batch_size, 3, 84, 84)))
        dummy_labels = np.random.randint(0, n_targets, (train_batch_size))
        if n_clients is None:
            train_loader_net = torch.utils.data.DataLoader(train_data, batch_size=1, collate_fn=numpy_collate, shuffle=True)
            test_loader_net = torch.utils.data.DataLoader(test_data, batch_size=1, collate_fn=numpy_collate, shuffle=False)
        ...
        train_data = datasets.CIFAR100('../data', train=True, download=True, transform=transforms.Compose([
                                                       # transforms.RandomCrop(32, padding=4), transforms.RandomHorizontalFlip(),
                                                       transforms.ToTensor(),
                                                       transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),
                                                       ToJNP()]))
        test_data = datasets.CIFAR100('../data', train=False, download=True, transform=transforms.Compose([
                                                        transforms.ToTensor(),
                                                        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),
                                                        ToJNP()]))
