...
def get_defend_grad(net, def_perturb_grads, batch_size, lr, fedavg_epochs, rand_batch):
    ...
    @jax.jit
    def nodefend_grad(rng, net_params, defense_params, inputs, targets):
        @jax.jit
        ...
        for j in range(fedavg_epochs):
            if rand_batch:
                order = np.random.permutation(inputs.shape[0])
                targets = targets[ order ]
                orders.append( orders[-1][order] if len(orders) > 0 else order )
                inputs = inputs[ order ]
            for i in range(k_batches):
                st_idx = i*batch_size
                en_idx = min( (i+1)*batch_size, inputs.shape[0] )
                grad = jax.vmap(single_grad, (0, 0, None))(inputs[st_idx:en_idx], targets[st_idx:en_idx], params)
                grad_avg = jax.tree_map(lambda x: jnp.mean(x, axis=0), grad)
                params = jax.tree_multimap(lambda p, g:  p - lr*g, params, grad_avg)
        if rand_batch:
            orders = jnp.stack( orders )
        else:
            orders = jnp.stack( [jnp.arange( inputs.shape[0] )]*fedavg_epochs )
        return jax.tree_multimap(lambda x_f, x_s: (x_f - x_s)/(-lr), params, net_params), orders
    ...
    def defend_grad(rng, net_params, defense_params, inputs, targets):
        ...
        for j in range(fedavg_epochs):
            ...
            for i in range(k_batches):
                st_idx = i*batch_size
                en_idx = min( (i+1)*batch_size, inputs.shape[0] )
                grad = jax.vmap(get_noisy_grad, (0, None, None, 0, 0))(perturb_rngs[st_idx:en_idx], params, defense_params, inputs[st_idx:en_idx], targets[st_idx:en_idx])
                grad_avg = jax.tree_map(lambda x: jnp.mean(x, axis=0), grad)
                params = jax.tree_multimap(lambda p, g:  p - lr*g, params, grad_avg)
        if rand_batch:
            orders = jnp.stack( orders )
            ...
            orders = jnp.stack( [jnp.arange( inputs.shape[0] )]*fedavg_epochs )
        noisy_grads = jax.tree_multimap(lambda x_f, x_s: (x_f - x_s)/(-lr), params, net_params)
        return noisy_grads, orders
    '''
    @jax.jit
    def defend_grad(rng, net_params, defense_params, inputs, targets):
        rngs = jax.random.split(rng, inputs.shape[0] + 1)
...
def random_pruning():
    ...
    def perturb_grads(rng, true_grads, defense_params, inputs, batched=True):
        @jax.jit
        def rng_drop(rng, ten, p):
            rand_in = jax.random.bernoulli(rng, 1-p, ten.shape).astype(float)
            ten = ten * rand_in
            return ten
        p = defense_params[0]
        num_tens = len(jax.tree_leaves(true_grads))
        rngs = jax.random.split(rng, num=num_tens)
        rngs = tree_unflatten(jax.tree_structure(true_grads), rngs)
        res = jax.tree_multimap(lambda g,r: rng_drop(r, g, p), true_grads, rngs)
        return res
