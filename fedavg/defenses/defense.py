...
from defenses.noise_defenses import dp_gaussian, dp_laplacian, random_pruning, soft_pruning, soft_gaussian_pruning, learned, learned_net, DefenseMLP
...
def get_defense(defense_name, rng, batch_size, dummy_input, dummy_grad):
    ...
    elif tokens[0] == 'random' and tokens[1] == 'pruning':
        p = float(tokens[2])
        def_perturb_grads, def_log_prob = random_pruning()
        return None, def_perturb_grads, def_log_prob, (p, )
